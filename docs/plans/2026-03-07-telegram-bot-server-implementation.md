...
class TestCommandRegistry:
    ...
    def test_suggest_heroes_has_skill_file(self):
        ...
        assert cmd["skill_file"] == "skills/suggest-heroes/SKILL.md"
...
**Files:**
- Create: `skills/suggest-heroes/SKILL.md`
...
```bash
git add bot/skills.py skills/suggest-heroes/SKILL.md tests/test_skills.py
git commit -m "feat: add skill template loading and suggest_heroes skill"
```
...
from bot.handlers import suggest_heroes_handler, build_handlers
from agent.base import AgentClient
...
class TestSuggestCountersHandler:
    ...
    async def test_sends_agent_response_as_html(self, tmp_path):
        skill_file = tmp_path / "suggest_heroes.md"
        skill_file.write_text("Counter: {heroes}")

        agent = FakeAgent(response="<b>Pick Fanny</b>")

        handler_fn = suggest_heroes_handler(agent, str(skill_file))
        update, context = _make_update_and_context(
            "/suggest_heroes Lancelot, Pharsa"
        )
    ...
    async def test_missing_args_sends_usage(self, tmp_path):
        ...
        handler_fn = suggest_heroes_handler(agent, str(skill_file))
        update, context = _make_update_and_context("/suggest_heroes")
        context.args = []
    ...
    async def test_agent_error_sends_error_message(self, tmp_path):
        ...
        handler_fn = suggest_heroes_handler(agent, str(skill_file))
        update, context = _make_update_and_context("/suggest_heroes Lancelot")
    ...
    async def handler(
        ...
        if not args:
            await update.effective_message.reply_html(
                "Usage: /suggest_heroes Hero1, Hero2, Hero3, Hero4, Hero5"
            )
            return
...
def build_handlers(
    ...
    handler_map: dict[str, Callable[..., HandlerFunc]] = {
        "/suggest_heroes": suggest_heroes_handler,
    }
...
class TestCreateApp:
    def test_creates_application_with_handlers(self):
        ...
        with (
            ...
            mock_build.return_value = [("suggest_heroes", mock_handler_fn)]
...
- Verify: `skills/suggest-heroes/SKILL.md` exists
...
Run: `cat skills/suggest-heroes/SKILL.md`
