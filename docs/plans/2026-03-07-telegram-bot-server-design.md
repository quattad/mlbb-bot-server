...
**In scope (v1):**
- `/suggest_counters` command — user provides enemy hero lineup, bot suggests counter picks
...
│   └── claude.py            # Claude Agent SDK implementation
├── skills/
│   └── suggest_counters.md  # Prompt template with formatting instructions
...
1. User sends `/suggest_counters Lancelot, Pharsa, Tigreal, Bruno, Rafaela`
2. `handlers.py` parses the command, extracts hero names as a comma-separated string
3. `handlers.py` loads `skills/suggest_counters.md`, interpolates `{heroes}` placeholder
...
- Registers `/suggest_counters` command handler
